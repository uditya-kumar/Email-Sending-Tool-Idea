...
-- One lead, one sending mailbox — enforced here rather than only in the app.
--
-- With several Gmail accounts connected, new leads are distributed between them
-- (`shared/assign-account.ts`). A *follow-up*, though, is threaded onto the
-- conversation the opening email started: it carries that message's `threadId`,
-- `In-Reply-To`/`References` and subject, and all of those are per-mailbox.
-- A follow-up sent from a different account does not fail — it silently starts a
-- second thread, so the recipient sees two strangers inside one conversation,
-- which is both confusing and the exact shape of a spoofing attempt.
--
-- The application already assigns correctly (the launch route pins the account
-- onto the opening `sends` row, `enqueueNextStep` copies it from the parent, and
-- `threadingFor` refuses to send on a mismatch). This trigger exists because that
-- is an *invariant*, and an invariant living only in application code is one
-- release away from a caller that forgets — with no after-the-fact query that
-- could catch it, since by then the email has been delivered.
--
-- A trigger and not a CHECK: the rule spans rows of the same table (every send
-- for a lead must agree), which no CHECK can express and which would otherwise
-- need a denormalised `leads.gmail_account_id` kept in step by hand.
...
-- these rows (status, claimed_at, sent_at, the rendered body), and re-running the
